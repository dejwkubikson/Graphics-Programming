...
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	shadersGEO[0] = CreateShader(LoadShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	shadersGEO[1] = CreateShader(LoadShader(fragFile), GL_FRAGMENT_SHADER); // create fragment shader
	shadersGEO[2] = CreateShader(LoadShader(geomFile), GL_GEOMETRY_SHADER);

	for (unsigned int i = 0; i < NUM_GEO_SHADERS; i++)
	{
		glAttachShader(program, shadersGEO[i]); //add all our shaders to the shader program "shaders" 
	}
...
Shader::~Shader()
{
	...
	for (unsigned int i = 0; i < NUM_GEO_SHADERS; i++)
	{
		glDetachShader(program, shadersGEO[i]); //detach shader from program
		glDeleteShader(shadersGEO[i]); //delete the sahders
	}
